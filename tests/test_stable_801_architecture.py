...
class Stable801ArchitectureTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_sortformer_does_not_use_unread_pipes(self):
        ...
        self.assertIn('duration <= 360.0', text)

    def test_settings_reports_actual_engines(self):
        text = (self.root / "app/ui/pages/settings_page.py").read_text(encoding="utf-8")
        self.assertIn("Faster-Whisper Small", text)
        self.assertIn("NemoDiarizationService", text)
        self.assertNotIn("Community-1", text)
        self.assertNotIn("ECAPA", text)
    ...
    def test_build_is_pinned_and_conversion_is_isolated(self):
        ...
        self.assertIn("AUDITOR_IA_8.0.1_Setup.exe", text)
    ...
    def test_installer_upgrades_existing_80(self):
        ...
        self.assertIn('MyAppVersion "8.0.1"', text)
