...
$buildFolder = Join-Path $distRoot "AUDITOR_IA_7.2.1_BUILD"
...
$setup = Join-Path $releaseRoot "AUDITOR_IA_7.2.1_Setup.exe"
if (-not (Test-Path $setup)) { throw "Setup no generado." }

$bytes = (Get-Item $setup).Length
$gb = [Math]::Round($bytes / 1GB, 2)
Write-Host "AUDITOR_IA_7.2.1_Setup.exe: $gb GB"
